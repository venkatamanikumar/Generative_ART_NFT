...
def interpolate(start_color, end_color, factor: float):
    recepient = 1  - factor
    return (
        int(start_color[0] * recepient + end_color[0] * factor),
        int(start_color[1] * recepient + end_color[1] * factor),
        int(start_color[2] * recepient + end_color[2] * factor),
    )
...
if __name__ == "__main__":
    for i in range(1000):
        generate_art(f"Generative Art #{i}.jpeg")